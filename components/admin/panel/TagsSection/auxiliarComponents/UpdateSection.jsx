...
import { getAllProducts, getAllTags, updateTag } from "@/app/context/actions";
import Loader from "@/components/Loader";
import { useFormik } from "formik";
import React, { useContext, useState } from "react";
import { toast } from "sonner";

const UpdateSection = ({ tag, setShowUpdateTags }) => {
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      resetForm();
      await updateTag(values.id, values.name);
      await getAllTags(dispatch);
      await getAllProducts(dispatch);
      setShowUpdateTags(false);
          ...
          >
            ...
            <div className="space-x-4 flex flex-row">
              ...
              <button
                ...
                onClick={() => setShowUpdateTags(false)}
              >
                Cancelar
              </button>
            </div>
