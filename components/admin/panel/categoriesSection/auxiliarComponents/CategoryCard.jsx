...
const CategoryCard = ({ value, removeValue, fullName = false }) => {
  const [showUpdateCategories, setShowUpdateCategories] = useState(false);
  const handleRemoveCategory = () => {
    removeValue(value);
  };

  if (showUpdateCategories) {
    return (
      <UpdateSection
        setShowUpdateCategories={setShowUpdateCategories}
        category={value}
      />
    );
  }
  ...
  return (
    <div className="flex flex-row gap-2 justify-center items-center border border-blue-800 py-0.5 px-2  rounded-3xl text-blue-800 text-xs">
      <svg
        ...
        onClick={() =>
          toast.info(
            `Seguro que quieres borrar la categoria '${value.name}'?`,
            {
              action: {
                label: "Confirmar",
      ...
      <svg
        ...
        stroke="#0040AF"
        className="w-5 h-5 cursor-pointer"
        onClick={() => setShowUpdateCategories(true)}
      >
        <path
          strokeLinecap="round"
          strokeLinejoin="round"
